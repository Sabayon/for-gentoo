...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/Attic/portage-2.2.0_alpha84.ebuild,v 1.3 2012/03/21 21:17:35 zmedico dead $
...
pkg_setup() {
	...
	if ! use python2 && ! use python3 && ! compatible_python_is_selected ; then
		...
		if [ $success != 1 ] ; then
			...
			die "This version of portage requires at least python-2.6 to be selected as the default python interpreter (see \`eselect 
python --help\`)."
		fi
	fi
